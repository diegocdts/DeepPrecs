...
def workflow_deblending(label, inputfile, train_model):
    ...
    with segyio.open(inputfile, "r", ignore_geometry=True) as file:

        # dados sísmicos
        inputdata = segyio.collect(file.trace[:])  # (ns*nr, nt)

        # número de samples no tempo
        nt = inputdata.shape[1]

        # dt (vem em microsegundos)
        dt = file.bin[BinField.Interval] / 1e6

        # posições das fontes e receptores
        sx = np.array([file.header[i][TraceField.SourceX] for i in range(file.tracecount)])
        gx = np.array([file.header[i][TraceField.GroupX] for i in range(file.tracecount)])

    # número de fontes e receptores
    ns = len(np.unique(sx))
    nr = len(np.unique(gx))

    # espaçamentos
    ds = np.diff(np.unique(sx)).min()
    dr = np.diff(np.unique(gx)).min()
    ...
    # Blending
    Bop = BlendingContinuous(nt, nr, ns, dt, ignition_times1.astype("float32"), dtype=np.float32)
